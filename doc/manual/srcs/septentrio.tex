...
{\bfseries version}\\
This option specifies the RINEX version for the output, via the `-R' option to \cc{sbf2rin}.
The version is specified using the same syntax as \cc{sbf2rin} uses.\\
\textit{Example:}
\begin{lstlisting}
version = 3 
\end{lstlisting}
...
This option controls generation of a RINEX navigation files by \cc{sbf2rin} via its `-n' option.
